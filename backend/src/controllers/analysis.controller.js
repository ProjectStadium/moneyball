  ...
  try {
    ...
    if (role) {
      filteredByRole = freeAgents.filter(player => 
        ...
        (player.agent_usage && getAgentRole(player.agent_usage) === role)
      );
    }
...
/**
 * Count players by role based on agent usage and playstyle
 */
function countPlayersByRole(players) {
  ...
  players.forEach(player => {
    // First try to get role from playstyle
    if (player.playstyle && player.playstyle.role_percentages) {
      const primaryRole = Object.entries(player.playstyle.role_percentages)
        .sort((a, b) => b[1] - a[1])[0];
      
      if (primaryRole && primaryRole[1] >= 40) {
        roleCounts[primaryRole[0]]++;
      } else {
        roleCounts['Flex']++;
      }
    } 
    // Fallback to agent usage if playstyle is not available
    else if (player.agent_usage) {
      const agentRole = getAgentRole(player.agent_usage);
      if (agentRole) {
...
/**
 * Get role based on agent usage
 */
function getAgentRole(agentUsage) {
  ...
  // Find the most used agent
  const mostUsedAgent = Object.entries(agentUsage)
    .sort((a, b) => b[1] - a[1])[0];

  if (!mostUsedAgent) return null;

  // Find the role for the most used agent
  for (const [role, agents] of Object.entries(roleAgents)) {
    if (agents.includes(mostUsedAgent[0])) {
      return role;
    }
  }
