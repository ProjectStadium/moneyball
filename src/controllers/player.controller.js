...
import { db } from '../utils/database.js'; // Import from database.js to ensure db is initialized
...
export const getTopPlayers = async (req, res) => {
  try {
    const { stat = 'rating', limit = 10, is_free_agent } = req.query;
...
export const getFreeAgents = async (req, res) => {
  try {
    const { min_rating = 0, limit = 100, offset = 0 } = req.query;
